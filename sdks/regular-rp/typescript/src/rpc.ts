// CSIL-RPC over an injected socket transport with LinkKeys TLS key pins.
// The application uses this transport for its configured RP server. Claim
// verification also uses it to fetch issuer keys and revocations.
//
...
// library (`src/vendor/csilgen-transport/`). This file supplies the TCP socket,
// TLS pin verification, and application API-key field.
//
...
// key is exported as JWK to recover the raw 32-byte key. The code hashes those
// bytes in the same way as the DNS `fp=` and domain-key fingerprint formats.
...
import type { AsyncServiceTransport } from "../generated/client.async.gen.ts";
...
async function dialPinnedTls(
  transport: Transport,
  endpoint: DomainEndpoint,
  signal?: AbortSignal,
): Promise<tls.TLSSocket> {
  const rawSocket: net.Socket = await transport.dial(endpoint.tcpAddr, { signal });
  ...
  return new Promise<tls.TLSSocket>((resolve, reject) => {
    ...
    tlsSocket.once("error", (e) => {
      fail(new TlsError(`TLS handshake failed: ${e.message}`, { cause: e }));
    });
  });
}
...
export async function call(
  ...
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
    throw new TypeError("RPC timeoutMs must be a positive finite number");
  }
  const deadline = new AbortController();
  const onAbort = () => deadline.abort(options.signal?.reason);
  if (options.signal?.aborted) onAbort();
  else options.signal?.addEventListener("abort", onAbort, { once: true });
  const timer = setTimeout(() => deadline.abort(new RpcTimeoutError(timeoutMs)), timeoutMs);

  let socket: tls.TLSSocket | undefined;
  try {
    socket = await dialPinnedTls(transport, endpoint, deadline.signal);
    const onCallAbort = () => socket?.destroy(deadline.signal.reason);
    deadline.signal.addEventListener("abort", onCallAbort, { once: true });
    const request = new RpcRequest(service, op, payload);
    if (auth !== undefined) request.withAuth(auth);
    try {
      ...
      deadline.signal.removeEventListener("abort", onCallAbort);
    }
  } finally {
    clearTimeout(timer);
...
export interface PinnedRpcTransportOptions {
  tcpAddress: string;
  fingerprints: readonly string[];
  apiKey: string;
  socketTransport?: Transport;
  /** Maximum time for one complete RPC, including connect, TLS, write, and read. */
  requestTimeoutMs?: number;
}

/** A generated-client transport for one configured LinkKeys RP server. */
export class PinnedRpcTransport implements AsyncServiceTransport {
  private readonly endpoint: DomainEndpoint;
  private readonly apiKey: string;
  private readonly socketTransport: Transport;
  private readonly requestTimeoutMs: number;

  constructor(options: PinnedRpcTransportOptions) {
    if (options.apiKey.length === 0) {
      throw new ProtocolError("the RP API key must not be empty");
    }
    if (options.fingerprints.length === 0 || options.fingerprints.some((fp) => !/^[0-9a-fA-F]{64}$/.test(fp))) {
      throw new ProtocolError("the RP fingerprint set must contain only SHA-256 hex fingerprints");
    }
    this.endpoint = {
      tcpAddr: options.tcpAddress,
      fingerprints: [...options.fingerprints],
    };
    this.apiKey = options.apiKey;
    this.socketTransport = options.socketTransport ?? new NodeTransport();
    this.requestTimeoutMs = options.requestTimeoutMs ?? DEFAULT_REQUEST_TIMEOUT_MS;
  }

  call(service: string, op: string, req: Uint8Array): Promise<Uint8Array> {
    return this.callWithOptions(service, op, req);
  }
  ...
  callWithOptions(
    ...
    return call(this.socketTransport, this.endpoint, service, op, req, this.apiKey, {
 ...
 * revocation this client needs to see — and marks any key that a
 * quorum-verified sibling revocation certificate targets. The verifier can
 * then compare a stored claim's signed attestation time with that instant. A
 ...
 * revocation application) is `NoTrustedDomainKeysError` — also fail closed.
 */
export async function fetchDomainKeys(
  ...
  const respBytes = await call(transport, endpoint, "DomainKeys", "get-domain-keys", payload, undefined, options);
  ...
  // A relevant revocation cannot predate the key that it revokes. Ask for
  // all revocations since the oldest presented key was created. If a peer
  // supplies an invalid key time, use the Unix epoch and fail toward more
  // revocation data, not less.
  const createdTimes = trusted.map((key) => Date.parse(key.createdAt));
  const oldestCreatedAt = createdTimes.every(Number.isFinite)
    ? Math.min(...createdTimes)
    : 0;
  const since = new Date(oldestCreatedAt).toISOString();
  ...
  for (const cert of revResp.revocations) {
    if (verifyRevocationCertificate(cert, trusted, domain)) {
      trusted = trusted.map((key) => key.keyId === cert.targetKeyId
        ? { ...key, revokedAt: cert.revokedAt }
        : key);
    }
  }
