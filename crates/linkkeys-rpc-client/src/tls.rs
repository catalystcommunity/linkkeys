//! Client-side TLS for the LinkKeys protocol: generate a self-signed cert from a
//! domain Ed25519 key, and verify a peer's cert by pinning its public-key
//! fingerprint to a DNS-published `fp=` set (no CA chain). This is the trust
//! model both ends of a server-to-server connection use.
...
/// Build a client TLS config pinned to `expected_fingerprints`, presenting a
/// client certificate (for mutual TLS) when `client_cert` is `Some`. This is the
/// single entry point callers use once they've resolved fingerprints (from DNS
/// or a pin) and optionally loaded their own domain cert.
pub fn client_config(
    ...
    match client_cert {
        Some((cert_der, key_der)) => {
            build_client_config_with_cert(expected_fingerprints, cert_der, key_der)
        }
        None => build_client_config(expected_fingerprints),
    }
}
...
mod tests {
    ...
    fn keypair() -> (Vec<u8>, [u8; 32]) {
        ...
        (vk.as_bytes().to_vec(), sk.to_bytes())
    }
    ...
    fn test_generate_cert_fingerprint_matches() {
        let (vk_bytes, seed) = keypair();
        let expected_fp = liblinkkeys::crypto::fingerprint(&vk_bytes);
        ...
        let cert_fp = liblinkkeys::crypto::fingerprint(&pub_key_bytes);
    ...
    fn test_fingerprint_verifier_accepts_matching() {
        let (vk_bytes, seed) = keypair();
        let fp = liblinkkeys::crypto::fingerprint(&vk_bytes);
